# compared to 'library(InvariantCausalPrediction)', this function outputs all p-values

ICPnew <- function (X, Y, ExpInd, alpha = 0.01, test = "normal", selection = c("lasso", 
    ...
    showAcceptedSets = TRUE, showCompletion = TRUE, stopIfEmpty = FALSE, gof= max(0.01,alpha)) 
{
    ...
    # small issue: 
    # original 'library(InvariantCausalPrediction)' seems have issue in reporting the finishing-percentage of the job: we correct it here
    #printoutat <- ifelse(length(testsets) > 0, c(2^(1:ceiling(log2(length(testsets))))), NA)
    printoutat <- if (length(testsets) > 0) c(2^(1:ceiling(log2(length(testsets))))) else NA
    ...
    retobj <- list(ConfInt = ConfInt, maximinCoefficients = maximin, 
        ...
        allsets = testsets, pvalues_allsets = Pall)
    # compared to 'library(InvariantCausalPrediction)', output all p-values
